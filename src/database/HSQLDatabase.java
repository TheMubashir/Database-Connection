...
public class HSQLDatabase implements Database {
	...
	public static Connection createHSQLDBConnection() throws SQLException {
		...
		try {
			...
			pds.setURL(
					"jdbc:hsqldb:file:C:\\Users\\Mubashir Ahmed\\Documents\\Database\\Database; hsqldb.lock_file = false; shutdown = true;");
	...
	public int queryChangeUsername(Integer id, String username) {
		...
		try {
			...
			if (rs.next()) {
				...
				return -1; // If username already exists in the database than return -1
	...
	public int queryChangeEmail(Integer id, String email) {
		...
		try {
			...
			if (rs.next()) {
				...
				return -1; // If email already exists in the database then return -1
	...
	public int queryChangePassword(Integer id, String password) {
		...
		try (PreparedStatement ps = connection.prepareStatement("UPDATE USERS SET Password = ? WHERE ID = ?");) {
